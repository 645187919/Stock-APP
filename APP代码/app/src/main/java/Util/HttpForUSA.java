    ...
    public static void parseJSONWithJSONObject(Context context, String jsonData) {
        ...
        try {
            ...
            for (int i = 0 ;i < jsonArray.length();i++){//循环遍历jsonArray
                ...
                stcokBean.newPrice=news_json.getString("price");



                stcokBean. symbol = news_json.getString("symbol");
                stcokBean. name = news_json.getString("cname");
                stcokBean. priceChange = news_json.getString("diff");
                stcokBean. changePercent = news_json.getString("chg");
